        ...
        while not rospy.is_shutdown():
            ...
            if self.radius:
                ...
                '''if self.image_received == 1:
                    ...
                    seg_img = self.bridge_object1.cv2_to_imgmsg(res_red, encoding = "passthrough")
                    self.image_pub.publish(seg_img)
                    seg_img = self.bridge_object1.cv2_to_imgmsg(res_green, encoding = "passthrough")
                    self.image_pub.publish(seg_img)
                    seg_img = self.bridge_object1.cv2_to_imgmsg(res_yellow, encoding = "passthrough")
                    self.image_pub.publish(seg_img)
