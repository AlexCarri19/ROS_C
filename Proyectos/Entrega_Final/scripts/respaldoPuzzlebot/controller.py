...
class Controller():
    def __init__(self):
        ...
        while not rospy.is_shutdown():
            ...
            if self.radius:
                ...
                t = rospy.get_time()
                t = rospy.get_time() - t
                if self.semaforo == "Verde" or self.semaforo == "Amarillo" or self.semaforo == "Nada":
                    self.center("l")
                ...
                print("Fuera de scope")
            
                
                """if f_radio == 1:
                    previous_time1 = rospy.get_time() #lo actualizo luego de usarlo
                    self.delta_t = rospy.get_time()-previous_time1
                    print("Delta t= " + str(self.delta_t))
                    
                    while 1<=self.delta_t<=2:
                        print("avanzando")
                        self.v_msg.linear.x = 0.05
                        self.v_msg.angular.z = 0.0

                    while 2<=self.delta_t<=5:
                        print("girando")
                        self.v_msg.linear.x = 0.0
                        w1 = (np.pi/2)/3 #angulo/tiempo
                        self.v_msg.angular.z = w1
    ...
    def center(self , turn):
        ...
        self.pub_cmd_vel.publish(self.v_msg)
        time.sleep(1)
        if turn == "r": self.turn_left()
        elif turn == "l": self.turn_right()
