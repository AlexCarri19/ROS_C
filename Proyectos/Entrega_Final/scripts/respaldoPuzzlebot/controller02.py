...
class Controller():
    ...
    def giros(self):
        ...
        if self.semaforo == "Verde" or self.semaforo == "Amarillo" or self.semaforo == "Nada" and self.bandera_giro == 0:
            self.center("l")
            self.bandera_giro = 1
            print("Girando a la izquierda")
            print("bandera: " + str(self.bandera_giro))
            self.estado = "Move"
            if not self.radius and self.estado == "Move":
                self.v_msg.linear.x = 0.0
                self.v_msg.angular.z = 0.03
            else:
                self.semaforoAct()

            print(self.estado)

        elif self.semaforo == "Verde" or self.semaforo == "Amarillo" or self.semaforo == "Nada" and self.bandera_giro == 1:
            self.center("r")
            print("Girando a la derecha")
            self.bandera_giro = self.bandera_giro + 1
            self.estado = "Move"
            self.semaforoAct()
            print(self.estado)
            print("bandera: " + str(self.bandera_giro))

        elif self.semaforo == "Verde" or self.semaforo == "Amarillo" or self.semaforo == "Nada" and self.bandera_giro == 2:     
            print("Sigo derecho")
            self.center("f")
            self.bandera_giro = 0
            self.estado = "Move"
    ...
    def turn_left(self):
        ...
        self.v_msg.angular.z = np.radians(90)
        self.pub_cmd_vel.publish(self.v_msg)
        time.sleep(1)
        self.v_msg.linear.x = 0.1
        self.v_msg.angular.z = 0.0
